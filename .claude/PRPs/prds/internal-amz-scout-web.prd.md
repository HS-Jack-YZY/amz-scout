...
| 3.5 | **Browser route audit + token safety gate** | **(新增 per council B−, 2026-04-24, ~2.5h)** (a) 30 分钟审计 `competitive_snapshots` 表每个字段的 Keepa 可覆盖率，产出 `docs/browser-route-audit.md`（含字段清单 + 覆盖率 + 降级建议）；(b) 为 `ensure_keepa_data` 添加 Chainlit 确认对话框，消费 `amz_scout.api` 已有的 `phase="needs_confirmation"` 协议。回答 Q8，堵 Alpha 前唯一的 token 烧光风险点。 | pending | - | 2 | - |
