...
class TestPauseTurnHandling:
    ...
    def test_container_id_threaded_across_iterations(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """When a response carries a ``container`` (from server_tool_use like
        web_search_20260209), the NEXT messages.create() call MUST pass that
        ``container_id`` back. Anthropic otherwise 400s with "container_id is
        required when there are pending tool uses generated by code execution
        with tools." — which was the real cause of an early-integration
        bug that broke every web_search conversation in 9c61eb3..07d5b1a
        until container threading was added.
        """
        _set_fake_env(monkeypatch)
        _reset_webapp_modules()
        from webapp import llm as webapp_llm
        ...
        class _Resp:
            def __init__(self, content: list, stop_reason: str,
                         container: _Container | None = None) -> None:
                self.content = content
                self.stop_reason = stop_reason
                self.usage = _Usage()
                self.container = container

        # First response: server_tool_use with a container. Second: end_turn.
        srv_block = _Block(
            type="server_tool_use",
            id="srv_xyz",
            name="web_search",
            input={"query": "site:amazon.co.uk router"},
        )
        text_block = _Block(type="text", text="done")
        responses = iter(
            [
                _Resp([srv_block], "pause_turn", container=_Container("ctr_abc123")),
                _Resp([text_block], "end_turn"),
            ]
        )

        captured_containers: list[str | None] = []

        def _fake_create(**kwargs):
            # Record the container kwarg Anthropic would see on each call.
            captured_containers.append(kwargs.get("container"))
            return next(responses)

        monkeypatch.setattr(webapp_llm._client.messages, "create", _fake_create)

        asyncio.run(
            webapp_llm.run_chat_turn([{"role": "user", "content": "find router"}])
        )

        assert len(captured_containers) == 2, (
            f"Expected exactly 2 create() calls, got {len(captured_containers)}"
        )
        assert captured_containers[0] is None, (
            "First call must not pass a container — no prior server-tool context"
        )
        assert captured_containers[1] == "ctr_abc123", (
            f"Resume call must pass container_id from first response's "
            f"resp.container.id; got {captured_containers[1]!r}. If this "
            f"fails, any web_search conversation will 400 on the next turn."
        )
